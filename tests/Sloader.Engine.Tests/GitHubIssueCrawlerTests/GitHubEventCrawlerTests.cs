...
namespace Sloader.Engine.Tests.GitHubIssueCrawlerTests
{
    public class GitHubIssueCrawlerTests
    {
        ...
        {
            string responseData = TestHelperForCurrentProject.GetTestFileContent($"GitHubIssueCrawlerTests/Sample/repo.json");

            return responseData;
        }

        private static async Task<GitHubIssueResult> InvokeSut(string repo, bool includeRaw = false)
        {
            string responseData = GetTestFileContent();

            var messageResponse = FakeHttpMessageHandler.GetStringHttpResponseMessage(responseData);

            var fakeMessageHandler = new FakeHttpMessageHandler(new HttpMessageOptions() { HttpResponseMessage = messageResponse, RequestUri = $"https://api.github.com/repos/{repo}/issues" });
